...
var query = 'SELECT rowid as row_id, lid,lon as longitude,lat as latitude FROM map_cd_below';
...
function update(i){
  ...
  var options = {
    coordinates:[gcj02towgs84(parseFloat(data_layer.traj[i].longitude), parseFloat(data_layer.traj[i].latitude))],
    number:1
  }
  engine.nearest(options,function(err,response){
    ...
    query = 'update map_cd_below set longitude=? where rowid=?';
    stmt = data_layer.database.taxi.prepare(query);
    stmt.run(lon,data_layer.traj[data_layer.i].row_id);
    query = 'update map_cd_below set latitude=? where rowid=?';
    stmt = data_layer.database.taxi.prepare(query);
