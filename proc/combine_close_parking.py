...
c.execute('select lid,latitude,longitude,type,confidence,bearing_before,bunch_type from signs_chengdu_sel_parking')
lid_list=c.fetchall()
S=[]
dis_thre=5
for row in lid_list:
...
while(dmin<=dis_thre and len(S)>1):
    ...
    dmin,p1_inx,p2_inx=find_closest(S)
for p in S:
    conn.execute('insert into parking_segments_chengdu(lid,latitude,longitude,bearing_before,confidence,bunch_type) values(?,?,?,?,?,?)',(p.lid,p.lat,p.lon,p.bb,p.conf,p.bunch_type))
    conn.commit()
